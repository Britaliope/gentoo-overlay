...
qt5-build_src_prepare() {
	...
	if [[ ${QT5_MODULE} == qtbase ]]; then
		...
		# Fix PKG_CONFIG_LIBDIR autodetection
		sed -i -e "s|\!exists(\"\$\$sysroot/usr/lib/pkgconfig\")|\!exists(\"\$\$sysroot/usr/lib/pkgconfig\"):\!exists(\"\$\$sysroot/usr/share/pkgconfig\")|" configure.pri || die
...
qt5-build_src_install() {
	...
	cp --recursive ${D}/${SYSROOT}/* ${D} && rm -rf ${D}/${SYSROOT}
...
# @FUNCTION: qt5_prepare_mkspecs
# @INTERNAL
# @DESCRIPTION:
# Create mkspecs files for building qmake.
qt5_prepare_mkspecs() {
	mkdir mkspecs/${CBUILD}
	{
		...
		if use kernel_FreeBSD; then
			if tc-is-gcc; then
				echo "include(../freebsd-g++/qmake.conf)"
			elif tc-is-clang; then
				echo "include(../freebsd-clang/qmake.conf)"
			fi
		fi
		if use kernel_linux; then
			if tc-is-gcc; then
				...
				echo "include(../linux-clang/qmake.conf)"
			fi
		fi

		echo "QMAKE_AR=$(tc-getBUILD_AR) cqs"
		echo "QMAKE_CC=$(tc-getBUILD_CC)"
		echo "QMAKE_LINK_C=$(tc-getBUILD_CC)"
		echo "QMAKE_LINK_C_SHLIB=$(tc-getBUILD_CC)"
		echo "QMAKE_CXX=$(tc-getBUILD_CXX)"
		echo "QMAKE_LINK=$(tc-getBUILD_CXX)"
		echo "QMAKE_LINK_SHLIB=$(tc-getBUILD_CXX)"
		echo "QMAKE_OBJCOPY=$(tc-getBUILD_OBJCOPY)"
		echo "QMAKE_NM = $(tc-getBUILD_NM) -P"
		echo "QMAKE_RANLIB="
		echo "QMAKE_STRIP=$(tc-getBUILD_STRIP)"
		echo "QMAKE_PKG_CONFIG = $(tc-getBUILD_PKG_CONFIG)"
		echo "QMAKE_CFLAGS=${CFLAGS_FOR_BUILD:-$CFLAGS}"
		echo "QMAKE_CFLAGS_RELEASE="
		echo "QMAKE_CFLAGS_DEBUG="
		echo "QMAKE_CXXFLAGS=${CXXFLAGS_FOR_BUILD:-$CXXFLAGS}"
		echo "QMAKE_CXXFLAGS_RELEASE="
		echo "QMAKE_CXXFLAGS_DEBUG="
		echo "QMAKE_LFLAGS=${LDFLAGS_FOR_BUILD:-$LDFLAGS}"
		echo "QMAKE_LFLAGS_RELEASE="
		echo "QMAKE_LFLAGS_DEBUG="

		echo "load(qt_config)"
	} > mkspecs/${CBUILD}/qmake.conf

	{
		if use kernel_FreeBSD; then
			if tc-is-gcc; then
				echo "#include \"../freebsd-g++/qplatformdefs.h\""
			elif tc-is-clang; then
				echo "#include \"../freebsd-clang/qplatformdefs.h\""
			fi
		fi
		if use kernel_linux; then
			if tc-is-gcc; then
				...
				echo "#include \"../linux-clang/qplatformdefs.h\""
			fi
		fi
	} > mkspecs/${CBUILD}/qplatformdefs.h
	...
	if tc-is-cross-compiler; then
		mkdir mkspecs/${CHOST}
		{
			...
			echo "QMAKE_AR=$(tc-getAR) cqs"
			echo "QMAKE_CC=$(tc-getCC)"
			echo "QMAKE_LINK_C=$(tc-getCC)"
			echo "QMAKE_LINK_C_SHLIB=$(tc-getCC)"
			echo "QMAKE_CXX=$(tc-getCXX)"
			echo "QMAKE_LINK=$(tc-getCXX)"
			echo "QMAKE_LINK_SHLIB=$(tc-getCXX)"
			echo "QMAKE_OBJCOPY=$(tc-getOBJCOPY)"
			echo "QMAKE_NM = $(tc-getNM) -P"
			echo "QMAKE_RANLIB="
			echo "QMAKE_STRIP=$(tc-getSTRIP)"
			echo "QMAKE_PKG_CONFIG = $(tc-getPKG_CONFIG)"
			...
			echo "load(qt_config)"
		} > mkspecs/${CHOST}/qmake.conf

		{
			echo '#include "../linux-g++/qplatformdefs.h"'
		} > mkspecs/${CHOST}/qplatformdefs.h
	fi
}
